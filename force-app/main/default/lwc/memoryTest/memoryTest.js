...
export default class MemoryTest extends LightningElement {
    ...
    cards= [
      ...
      get rateGame(){
        ...
        return this.matchC.length ===16 ? stars :[]
        }
      displayCard(event){
          let currC = event.target
          currC.classList.add("open", "show", "disabled")
          this.openedC = this.openedC.concat(event.target)
          const len = this.openedC.length
          if(len === 2){
            this.moves = this.moves+1
            if(this.moves === 1){
                this.timer()
            }
            if(this.openedC[0].type === this.openedC[1].type){
                this.matchC = this.matchC.concat(this.openedC[0], this.openedC[1])
                this.matched()
            } else {
                this.unmatched()
            }
      ...
      matched(){
          this.openedC[0].classList.add("match", "disabled")
          this.openedC[1].classList.add("match", "disabled")
          this.openedC[0].classList.remove("show", "open")
          this.openedC[1].classList.remove("show", "open")
          this.openedC=[]
          if(this.matchC.length === 16){
              window.clearInterval(this.timerRef)
              this.congrat = true
          }
      }
      unmatched(){
        this.openedC[0].classList.add("unmatched")
        this.openedC[1].classList.add("unmatched")
        this.action('DISABLE')
        setTimeout(()=>{
            this.openedC[0].classList.remove("show", "open", "unmatched")
            this.openedC[1].classList.remove("show", "open", "unmatched")
            this.action('ENABLE')
            this.openedC=[]
        },1100)
      }
